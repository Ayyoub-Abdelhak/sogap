...
class YetiForcePDF extends Base
{
	 ...
	 * @return string
	 */
	public function getExcelHtml(): string
	{
		$html .= $this->header ? $this->wrapHeaderContent($this->header) : '';
		$html .= $this->getBody();
		$html .= $this->footer ? $this->wrapFooterContent($this->footer) : '';
	...
	public function loadWatermark()
	{
		...
		if (self::WATERMARK_TYPE_IMAGE === $this->template->get('watermark_type') && '' !== trim($this->template->get('watermark_image'))) {
			if ($this->template->get('watermark_image')) {
				$this->watermark = '<img src="' . $this->template->get('watermark_image') . '" style="opacity:1;">';
			}
		} elseif (self::WATERMARK_TYPE_TEXT === $this->template->get('watermark_type') && '' !== trim($this->template->get('watermark_text'))) {
			$this->watermark = '<div style="opacity:1;display:inline-block;">' . $this->template->parseVariables($this->template->get('watermark_text')) . '</div>';
		}
		return $this;
	}
	...
	/** {@inheritdoc} */
	public function outputExcel($fileName = '', $mode = 'D'): void
	{
		if (empty($fileName)) {
			$fileName = ($this->getFileName() ?: time()) . '.xlsx';
		}

		$reader = new \PhpOffice\PhpSpreadsheet\Reader\Html();
		$spreadsheet = $reader->loadFromString($this->getExcelHtml());
		...
		// Clear all previously output data
		ob_end_clean();

		// Prepare to output directly to the browser
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="' . \App\Fields\File::sanitizeUploadFileName($fileName) . '"');
