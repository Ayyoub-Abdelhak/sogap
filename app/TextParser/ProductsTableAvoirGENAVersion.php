...
/**
 * Products table avoir GENA version (retenue de garantie on TTC).
 *
 * @package TextParser
 *
 ...
 */
class ProductsTableAvoirGENAVersion extends Base
{
    ...
    public function process()
    {
        ...
        if (!empty($groupModels)) {
            ...
            $retenueGarantiePercentage = intval($invoiceModel->get('retenue_garantie'));
            ...
            if ($retenueGarantie > 0) {
                $html .= '
				<tr>
					<td style="width:75%;text-align:center;border:1px solid black;">RETENUE DE GARANTIE ' . $retenueGarantiePercentage . '%</td>
					<td style="width: 25%;text-align:center;border:1px solid black;">' . \CurrencyField::convertToUserFormat($retenueGarantie, null, true) . '</td>
				</tr>
				<tr>
					<td style="width:75%;text-align:center;border:1px solid black;">NET À PAYER</td>
					<td style="width: 25%;text-align:center;border:1px solid black;">' . \CurrencyField::convertToUserFormat($totalRGTTC, null, true) . '</td>
				</tr>';
            }
            $html .= '
				</table>
            ...
            $finalAmount = $retenueGarantie > 0 ? $totalRGTTC : $totalTTC;
            $currency = \Vtiger_Util_Helper::is_decimal($finalAmount) ? '' : ' DIRHAMS';
            $html .= '<div style="font-size:9px;margin-top:20px;"><b>ARRÊTÉ LE PRÉSENT AVOIR À LA SOMME DE :</b><br /><b>' . \Vtiger_Util_Helper::int2str($finalAmount) . $currency . ' TTC</b></div>';
        }
        return $html;
    }
}
