 ...
 */
class ProductsTableCommandeFournisseurVersion extends Base
{
	...
	public function process()
	{
		...
		foreach (['ItemNumber', 'Name', 'Value', 'UnitPrice', 'Quantity', 'TotalPrice', 'Tax', 'GrossPrice']  as $fieldType) {
			foreach ($inventory->getFieldsByType($fieldType) as $fieldModel) {
				...
				if($typeName == 'Quantity') $headerName = 'Qte';
				if($typeName == 'UnitPrice') $headerName = 'Prix U.';
				if($typeName == 'TotalPrice') $headerName = 'HT';
				if($typeName == 'GrossPrice') $headerName = 'TTC';
