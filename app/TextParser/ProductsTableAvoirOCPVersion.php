...
/**
 * Products table avoir OCP version (reception definitive + provisoire).
 *
 * @package TextParser
 *
 ...
 */
class ProductsTableAvoirOCPVersion extends Base
{
    ...
    public function process()
    {
        ...
        if (!empty($groupModels)) {
            ...
            foreach ($groupModels as $fieldModel) {
                ...
                $html .= "<th style=\"{$headerStyle}\"></th>";
            }
            $html .= '</tr></tfoot></table>';

            $invoiceModel = \Vtiger_Record_Model::getInstanceById($this->textParser->recordModel->get('finvoiceid'));
            $recordDiscount = (float)($invoiceModel->get('discount') ?? 0);
            $remise = $discount + $recordDiscount;
            $totalHT = $ht - $remise;
            $receptionDefinitivePercentage = intval($invoiceModel->get('reception_definitive'));
            $receptionProvisoirePercentage = intval($invoiceModel->get('reception_provisoire'));
			...
			<div style="padding: 0px 0px 0px 221px">
				<table style="width:100%;font-size:8px;margin-top:15px;border:1px solid black;font-weight:bold;border-collapse:collapse;border:1px solid black;">
					<tr>
						<td style="width:75%;text-align:center;border:1px solid black;">TOTAL HT</td>
						<td style="width: 25%;text-align:center;border:1px solid black;">' . \CurrencyField::convertToUserFormat($ht, null, true) . '</td>
					</tr>';
            if ($remise > 0) {
                $html .= '
				<tr>
					<td style="width:75%;text-align:center;border:1px solid black;">REMISE</td>
            ...
            if ($receptionProvisoire > 0) {
                $html .= '
				<tr>
					<td style="width:75%;text-align:center;border:1px solid black;">RÉCEPTION PROVISOIRE ' . $receptionProvisoirePercentage . '%</td>
					<td style="width: 25%;text-align:center;border:1px solid black;">' . \CurrencyField::convertToUserFormat($receptionProvisoire, null, true) . '</td>
				</tr>';
            }
            if ($receptionDefinitive > 0) {
                $html .= '
				<tr>
					<td style="width:75%;text-align:center;border:1px solid black;">RÉCEPTION DÉFINITIVE ' . $receptionDefinitivePercentage . '%</td>
					<td style="width: 25%;text-align:center;border:1px solid black;">' . \CurrencyField::convertToUserFormat($receptionDefinitive, null, true) . '</td>
				</tr>';
            }
            if ($receptionDefinitive > 0 || $receptionProvisoire > 0) {
				...
				<tr>
					<td style="width:75%;text-align:center;border:1px solid black;">TOTAL HT</td>
					<td style="width: 25%;text-align:center;border:1px solid black;">' . \CurrencyField::convertToUserFormat($totalHT, null, true) . '</td>
				</tr>';
            }
            if ($totalTVA > 0) {
                $html .= '
				<tr>
					<td style="width:75%;text-align:center;border:1px solid black;">TVA 20%</td>
					<td style="width: 25%;text-align:center;border:1px solid black;">' . \CurrencyField::convertToUserFormat($totalTVA, null, true) . '</td>
				</tr>';
            }
            $html .= '
					<tr>
						<td style="width:75%;text-align:center;border:1px solid black;">TOTAL TTC</td>
            ...
            $html .= '<div style="font-size:9px;margin-top:20px;"><b>ARRÊTÉ LE PRÉSENT AVOIR À LA SOMME DE :</b><br /><b>' . \Vtiger_Util_Helper::int2str($totalTTC) . $currency . ' TTC</b></div>';
        }
        return $html;
    }
}
