 ...
 */
class Vtiger_EXCEL_Action extends \App\Controller\Action
{
		...
		foreach ($recordIds as $recordId) {
			foreach ($templateIds as $templateId) {
				...
				if ($attach || $emailPdf || ($countTemplates > 1 || (1 === $countTemplates && !isset($skip[$templateId]) && $countRecords > 1))) {
					...
					$fileName .= ($increment[$fileName]++ > 0 ? '_' . $increment[$fileName] : '') . '.xlsx';
				...
				//dd($pdf);
				$pdf->outputExcel($filePath, $mode ?? $pdfFlag ?? 'D');
			}
		}
		if ($singlePdf) {
			\App\Pdf\Pdf::merge(array_column($pdfFiles, 'path'), \App\Fields\File::sanitizeUploadFileName(\App\Language::translate('LBL_PDF_MANY_IN_ONE')) . '.xlsx', $pdfFlag ?: 'D');
