...
class Conge_Edit_View extends Vtiger_Edit_View
{
	...
	public function process(App\Request $request)
	{
		...
		$soldeFN1 = (float)$this->record->get('soldefinnmoins1');
		$droitCp = (float)$this->record->get('droit_cp_annee_encours');
		$soldeInitial = $soldeFN1 + $droitCp;
		$janvier = (float)$this->record->get('janvier');
		$fevrier = (float)$this->record->get('fevrier');
		$mars = (float)$this->record->get('mars');
		$avril = (float)$this->record->get('avril');
		$mai = (float)$this->record->get('mai');
		$juin = (float)$this->record->get('juin');
		$juillet = (float)$this->record->get('juillet');
		$aout = (float)$this->record->get('aout');
		$septembre = (float)$this->record->get('septembre');
		$octobre = (float)$this->record->get('octobre');
		$novembre = (float)$this->record->get('novembre');
		$decembre = (float)$this->record->get('decembre');
		...
		$this->record->set('solde_initial', $soldeInitial);
		$this->record->set('consommations_cp_n', $consomations);
		$this->record->set('solde_cp_n', $soldeCP);
